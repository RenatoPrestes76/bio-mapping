...
export class MilestonePredictionEngine {
  ...
  private fromGoals(
    ...
    const onTrackGoals = goals.filter(
      (g) => g.isOnTrack() && g.progressPercent >= 50 && !g.isCompleted(),
    );
    ...
    for (const goal of onTrackGoals.slice(0, 2)) {
      ...
      const confidence: PredictionConfidence =
        goal.progressPercent >= 70 ? 'HIGH' : 'MODERATE';
      ...
      predictions.push(
        new MilestonePrediction({
          ...
          confidence,
          requiredActions: [
            `Manter consistência em: ${goal.category}`,
            ...goal.evidences.slice(0, 1),
          ],
    ...
    const nearlyAchieved = goals.filter(
      (g) => g.progressPercent === 100 && g.status !== 'ACHIEVED',
    );
    for (const goal of nearlyAchieved.slice(0, 1)) {
      predictions.push(
        new MilestonePrediction({
          ...
          description:
            'Meta praticamente atingida — aguarda validação clínica para reconhecimento.',
          category: 'GOAL_ACHIEVEMENT',
          estimatedTimeframe: 'Próximas 1–2 semanas',
          confidence: 'HIGH',
          requiredActions: [
            'Confirmar com seu médico que a meta foi alcançada',
          ],
  ...
  private fromScoreTrend(
    ...
    const insight = interpretTrend(last3.map((p) => p.score));
    if (insight.direction !== 'IMPROVING') return [];
    ...
    return [
      new MilestonePrediction({
        ...
        requiredActions: [
          'Manter rotina de consultas e exames',
          'Registrar todos os eventos de saúde',
        ],
  ...
  private fromHabits(
    ...
    const improvingHabits = habitPatterns.filter(
      (h) => h.trend === 'IMPROVING' && h.consistencyScore >= 50,
    );
    ...
    return [
      new MilestonePrediction({
        ...
        requiredActions: improvingHabits
          .map((h) => `Manter: ${h.label}`)
          .slice(0, 2),
  ...
  private routineFollowUp(patientId: string): MilestonePrediction {
    return new MilestonePrediction({
      ...
      description:
        'Manter a periodicidade de consultas é o alicerce de toda jornada de saúde bem-sucedida.',
