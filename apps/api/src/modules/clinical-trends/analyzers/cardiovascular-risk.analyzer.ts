...
export class CardiovascularRiskAnalyzer implements IClinicalTrendAnalyzer {
  readonly metric = METRICS.CARDIOVASCULAR_RISK;

  supports(metric: string): boolean {
    return metric === METRICS.CARDIOVASCULAR_RISK;
  }
  ...
  analyze(input: { patientId: string; dataPoints: DataPoint[] }): TrendAnalysisResult {
    return computeTrend(input.dataPoints, {
      ...
      stableThresholdPerDay: 0.05,
      summaryLabel: 'Risco cardiovascular',
    });
  }
  ...
  buildTrend(patientId: string, result: TrendAnalysisResult, createdBy?: string): CreateTrendData {
    return {
      ...
      sourceModule: 'patient-monitoring',
